			     ...
			     const char *data, int len)
{
 ...
 static void sensor_get_prj_name(char *buf, size_t len)
 {
	 ...
	 if(!ret){
		 ...
		 modem_id = strstr(cmd_line, "rfboard.id=");
		 if (modem_id)
		 	modem_id += strlen("rfboard.id=");
		 else
		 	return;

		 strncpy(buf, temp_name, len - 1);
		 strncpy(buf + len -1, modem_id, 1);
	 }
 }
 ...
 int sensor_get_nfc_flag(struct shub_data *sensor)
 {
	 ...
	 static char prj_name[7];

	 sensor_get_prj_name(prj_name, sizeof(prj_name) - 1);
	 printk("%s: prj_name=%s\n", __func__, prj_name);
